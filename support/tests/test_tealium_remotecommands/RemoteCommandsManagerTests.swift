...
class RemoteCommandsManagerTests: XCTestCase {
    ...
    override func setUp() {
        ...
        tealiumRemoteJSONCommand = RemoteCommand(commandId: "example", description: "test", type: .remote(url: "https://tags.tiqcdn.com/dle/services-christina/tagbridge/example.json")) { _ in
            // ...
        }
    ...
    func testAddRemoteCommandCallsGetCachedConfig() {
        XCTAssertEqual(1, mockDiskStorage.retrieveCount, "Cache should be tried to be read on example remote command")
        ...
        XCTAssertEqual(2, mockDiskStorage.retrieveCount, "Cache should be tried to be read on both newId and example remote commands")
        TealiumQueues.backgroundSerialQueue.sync {
            XCTAssertEqual(2, mockDiskStorage.saveCount, "Save should be called twice for both ids")
    ...
    func testTriggerCommandWithoutPayload() {
        ...
        tealiumRemoteCommandsManager.remove(jsonCommand: "local")
        TealiumQueues.backgroundSerialQueue.sync {
            tealiumRemoteCommandsManager.jsonCommands.forEach { command in
                var command = command
                command.delegate = mockDelegate
