...
class DispatchQueueTests: XCTestCase {
    ...
    func testRemoveOldDispatches() {
        ...
        persistentQueue?.appendDispatch(TealiumTrackRequest(data: ["tealium_event": "current_date", TealiumDataKey.timestampUnix: date.unixTimeSeconds]))
        persistentQueue?.appendDispatch(TealiumTrackRequest(data: ["tealium_event": "no_timestamp"]))
        persistentQueue?.appendDispatch(TealiumTrackRequest(data: ["tealium_event": "old_date", TealiumDataKey.timestampUnix: timeTraveler.travel(by: daysInFuture(2)).unixTimeSeconds]))
        XCTAssertEqual(persistentQueue?.currentEvents, 3)
