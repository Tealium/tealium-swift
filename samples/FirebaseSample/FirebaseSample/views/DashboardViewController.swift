...
class DashboardViewController: UIViewController {
    ...
    enum DashboardViewControllerStrings {
    ...
    func toggleLoyaltyStatus(_ status: String?) -> String {
        if let status = status {
            return status == DashboardViewControllerStrings.basic ? DashboardViewControllerStrings.vip : DashboardViewControllerStrings.basic
        } else {
            return DashboardViewControllerStrings.basic
        }
    }
    
    @IBAction func setLoyaltyStatus(_ sender: Any) {
        if let currentLoyaltyStatus = UserDefaults.standard.object(forKey: DashboardViewControllerStrings.userLoyalty) as? String {
            loyaltyStatus = currentLoyaltyStatus
        }
        let toggle = toggleLoyaltyStatus(loyaltyStatus)
        
        /* trackEvent sent to Tealium with the set_loyalty_status event and associated parameters.
         This will then get mapped to trigger the setUserProperty(<toggle>, forName: "user_loyalty_status") method in Firebase */
        ...
        UserDefaults.standard.set(toggle, forKey: DashboardViewControllerStrings.userLoyalty)
    }
    
    @IBAction func addToCart(_ sender: Any) {
        let addToCartData = ["product_id": ["ABC123"], "product_price": ["14.99"], "product_quantity": ["1"], "product_name": ["Premium Widget"]]
        
        /* trackEvent sent to Tealium with the cart_add event and associated parameters.
        This will then get mapped in TiQ to trigger the logEvent("add_to_cart", firebase_event_params) method in Firebase */
